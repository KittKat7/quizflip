...
class _FlashcardPageState extends State<FlashcardPage> {
  ...
  bool get isEdited => isKeyEdited || isDeckEdited || isValueEdited || isTagsEdited;
  ...
  void _updateValue(String nValue, int i) {
    values[i] = nValue;
    isValueEdited = widget.cardValues.length != values.length || widget.cardValues[i] != values[i];
  ...
  void _actionSave() {
    Flashcard card =
      Flashcard(key: key, deck: deck, values: values, tags: tags, confidence: widget.cardCnfdnc);
    ...
    if (widget.card == null || widget.cardKey != key || widget.cardDeck != deck) {
  ...
  Widget build(BuildContext context) {
    ...
    SimpleText hdrKey = SimpleText(getLang('hint_create_new_card_key'), isBold: true);
    SimpleText hdrDeck = SimpleText(getLang('hint_create_new_card_deck'), isBold: true);
    SimpleText hdrValue = SimpleText(getLang('hint_create_new_card_values'), isBold: true);
    SimpleText hdrValuesFake =SimpleText(getLang('hint_create_new_card_values_fake'), isBold: true);
    SimpleText hdrTags = SimpleText(getLang('hint_create_new_card_tags'), isBold: true);
    ...
    if (valuesFakeField.isNotEmpty) {
      ...
      for (int i = 0; i < valuesFakeField.length; i++) {
        ...
        if (i != valuesFakeField.length) children.add(const SizedBox(height: 20));
      }//for
    }//if
    ...
    ElevatedButton closeButton = ElevatedButton(
    ...
    children.add(Padding(
      ...
      child: Wrap(children: [closeButton, deleteButton, editButton, saveButton])));

    Column column = Column(children: children);

    Aspect aspect = Aspect(child: Align(alignment: Alignment.topCenter, child: SingleChildScrollView(child: column)));

    AppBar appBar = AppBar(
      title: SimpleText(getLang('flashcard'), isBold: true,),
      centerTitle: true,);
