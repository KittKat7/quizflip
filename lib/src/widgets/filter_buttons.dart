...
class _DeckButtonsState extends State<DeckButtons> {
  ...
  void _updateFilters(List<Flashcard> fltrdCards) {
    ...
    for (Flashcard card in fltrdCards) {
      if (card.deck.startsWith(CardList.filter) && card.deck.toList().length > CardList.filter.length) {
  ...
  void initState() {
    ...
    filteredCardsStream.listen((event) { _updateFilters(event); });
  }//initState
  ...
  Widget build(BuildContext context) {
    ...
    for (String str in filterList) {
      buttons.add(
        ElevatedButton(
          onPressed: () => CardList.pushFilter(str),
          child: Text(str))
      );
    }

    return Align(alignment: Alignment.centerLeft, child: Wrap(children: buttons,));
  }
}//_DeckButtonsState