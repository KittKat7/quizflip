  ...
  Widget build(BuildContext context) {
    ...
    Aspect aspect = Aspect(child: SingleChildScrollView(
      child: FlashcardButton(
        ...
        state: review.isShowingValue? FlashcardButtonState.back : FlashcardButtonState.front,
        onPressed: () { review.flipCard(); setState((){}); },)));
    ...
    Widget resetConfidence(Widget child) {
      return LongPressWidget(
        child: child,
        onLongPress: () { review.setConfidence(-1); setState((){}); });
    }

    List<Widget> children = [];

    if (review.hasPrevCard()) {
      children.add(Expanded(child: resetConfidence(IconButton(
        onPressed: () { review.prevCard(); setState((){}); },
    ...
    children.addAll([
      Expanded(child: resetConfidence(IconButton(
        onPressed: () { review.setConfidence(0); setState((){}); },
        icon: Icon(review.confidence == 0? Icons.remove_circle: Icons.remove_circle_outline)),)),
      Expanded(child: resetConfidence(IconButton(
        onPressed: () { review.setConfidence(1); setState((){}); },
        icon: Icon(review.confidence == 1? Icons.circle : Icons.circle_outlined)),)),
      Expanded(child: resetConfidence(IconButton(
        onPressed: () { review.setConfidence(2); setState((){}); },
        icon: Icon(review.confidence == 2? Icons.add_circle: Icons.add_circle_outline)),)),
    ]);

    if (review.hasNextCard()) {
      children.add(Expanded(child: resetConfidence(IconButton(
        onPressed: () { review.nextCard(); setState((){}); },
      ...
      children.add(Expanded(child: resetConfidence(IconButton(
        onPressed: () { 
          confirmPopup(
            context,
            getLang('hdr_finish_review'),
            getLang('msg_finish_review'),
            () {
              Navigator.pop(context);
              Navigator.push(context, genRoute(_ReviewResultsPage(review: review,)));
            });
           },
    ...
    PopScope popScope = PopScope(
      ...
      onPopInvoked: (didPop) {
        ...
        confirmPopup(
          context,
          getLang('hdr_leave_review'),
          getLang('msg_leave_review'),
          () => Navigator.pop(context));
      },
      child: scaffold);
    return popScope;
